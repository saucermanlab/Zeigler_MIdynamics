function [InputCsim,tInSim,inputNode,resNorm,resNormConvert,weightR] = InputCurve_12_19rand(meanWeight, stdWeight)
...
weightR = normrnd(meanWeight,stdWeight,9,1);
...
FCmax_TGFB = max(y);
FCmin_TGFB = calc_FCmin(FCmax_TGFB, weightR(1), weightR(1)-0.1);

x = [FCmin_TGFB, 1, FCmax_TGFB*1.05];
y = [0, 0.1, weightR(1)*1.05]; 
...
while abs(max(yTGFB) - weightR(1)) > 0.00001
    %disp('TGFB');
    %disp(max(yTGFB));
    if (max(yTGFB) - weightR(1)) > 0
...
FCmax_IL6 = max(y);
FCmin_IL6 = calc_FCmin(FCmax_IL6, weightR(2), weightR(2)-0.1);

x = [FCmin_IL6, 1, FCmax_IL6*1.05];
y = [0, 0.1, weightR(2)*1.05]; 
...
while abs(max(yIL6) - weightR(2)) > 0.00001
    %disp('IL6');
    %disp(max(yIL6));
    if (max(yIL6) - weightR(2)) > 0
...
FCmax_IL1 = max(y);
FCmin_IL1 = calc_FCmin(FCmax_IL1, weightR(3), weightR(3)-0.1);

x = [FCmin_IL1, 1, FCmax_IL1*1.05];
y = [0, 0.1, weightR(3)*1.05]; 
...
while abs(max(yIL1) - weightR(3)) > 0.00001
    %disp('IL1');
    %disp(max(yIL1));
    if (max(yIL1) - weightR(3)) > 0
...
FCmax_TNFa = max(y);
FCmin_TNFa = calc_FCmin(FCmax_TNFa, weightR(4), weightR(4)-0.1);

x = [FCmin_TNFa, 1, FCmax_TNFa*1.05];
y = [0, 0.1, weightR(4)*1.05]; 
...
while abs(max(yTNFa) - weightR(4)) > 0.00001
    %disp('TNFa');
    %disp(max(yTNFa));
    if (max(yTNFa) - weightR(4)) > 0
...
FCmax_NE = max(y1_ne);
FCmin_NE = calc_FCmin(FCmax_NE, weightR(5), weightR(5)-0.1);

x = [FCmin_NE, 1, FCmax_NE*1.05];
y = [0, 0.1, weightR(5)*1.05]; 
...
while abs(max(yNE) - weightR(5)) > 0.00001
    %disp('NE');
    %disp(max(yNE));
    if (max(yNE) - weightR(5)) > 0
...
FCmax_ET1 = max(y1_et);
FCmin_ET1 = calc_FCmin(FCmax_ET1, weightR(6), weightR(6)-0.1);

x = [FCmin_ET1, 1, FCmax_ET1*1.05];
y = [0, 0.1, weightR(6)*1.05]; 
...
while abs(max(yET1) - weightR(6)) > 0.00001
    %disp('ET1');
    %disp(max(yET1));
    if (max(yET1) - weightR(6)) > 0
...
FCmin_BNP = calc_FCmin(FCmax_BNP, weightR(7), weightR(7)-0.1);

x = [FCmin_BNP, 1, FCmax_BNP*1.05];
y = [0, 0.1, weightR(7)*1.05]; 
...
while abs(max(yBNP) - weightR(7)) > 0.00001
    %disp('BNP');
    %disp(max(yBNP));
    if (max(yBNP) - weightR(7)) > 0
...
FCmax_Ang = max(y);
FCmin_Ang = calc_FCmin(FCmax_Ang, weightR(8), weightR(8)-0.1);

x = [FCmin_Ang, 1, FCmax_Ang*1.05];
y = [0, 0.1, weightR(8)*1.05]; 
...
while abs(max(yAng) - weightR(8)) > 0.00001
    %disp('Ang');
    %disp(max(yAng));
    if (max(yAng) - weightR(8)) > 0
...
FCmin_PDGF = calc_FCmin(FCmax_PDGF, weightR(9), weightR(9)-0.1);

x = [FCmin_PDGF, 1, FCmax_PDGF*1.05];
y = [0, 0.1, weightR(9)*1.05]; 
...
while abs(max(yPDGF) - weightR(9)) > 0.00001
    %disp('PDGF');
    %disp(max(yPDGF));
    if (max(yPDGF) - weightR(9)) > 0
